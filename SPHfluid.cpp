...
const float Kp = 3 / 0.004 / 0.004; // Pressure Stiffness
const float visc = 0.25 * 0.004; // Viscosity
const float tension = 150.0; // Surface Tension
const float dt = 0.004; // time step
...
float bound[3] = {100, 30, 60}; // from 0, 0, 0
...
void initial_scene(){
	part_n = 0;
	add_particles(vec(0, 0, 0), vec(30, 30, 30));
	//add_particles(vec(85, 0, 0), vec(100, 20, 30));
	construct_grid();
...
FILE *matlab = fopen("matlab_render", "w");
FILE *output = fopen("simulated_sph", "w");

int main(int argc, char **argv){
	if(argc != 2){
		fprintf(stderr, "./SPHfluid ROUND\n");
		exit(-1);
	}
	...
	for(int T = 0; T < ROUND; T ++){
		...
		for(int i = 0; i < part_n; i++) if(T % 2 == 0)
